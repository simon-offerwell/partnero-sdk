...
class PartnerAPI(BaseAPI):
    ...
    def create_partner(
        self, email: str, name: str, password: str, id: str = None, key: str = None
    ) -> dict:
        data = {
            "email": email,
            "name": name,
    ...
    def get_partner(
        self, email: str = None, name: str = None, partner_id: str = None
    ) -> dict:
        """
        ...
        params = {
            k: v
            for k, v in [("email", email), ("name", name), ("id", partner_id)]
            if v is not None
        }
